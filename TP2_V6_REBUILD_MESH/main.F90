program main
  ...
  ALLOCATE(TARG(5))
  TARG(2)%XY(1)=TXY(1);TARG(2)%XY(2)=TXY(2)
  TARG(1)%XY(1)=0.3;TARG(1)%XY(2)=0.31
  TARG(3)%XY(1)=0.125;TARG(3)%XY(2)=0.35
  TARG(4)%XY(1)=0.125;TARG(4)%XY(2)=0.25
  TARG(5)%XY(1)=0.125;TARG(5)%XY(2)=0.25
  CRTL_NOY = NOYDELAU(TARG)
  !DO i=1,size(CAV)
  !  PRINT*,'i, CAV',i,CAV(i)
  !END DO
  !DEALLOCATE(CAV,TARG)
  DEALLOCATE(TARG,T1CO, T1CN, T1ED, T1ED_L)

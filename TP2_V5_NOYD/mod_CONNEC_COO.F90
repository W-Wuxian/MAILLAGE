...
module mod_CONNEC_COO
  USE mod_parameters, ONLY : NT, PR, STCO, STCN, NBPELE, NBPEDG, quality
  IMPLICIT NONE
...
CONTAINS
  ...
  SUBROUTINE RD_MF_CONNEC_COO(MFILE,DIM,NV,NE,NT,T1CO,T1CN,T1ED,T1ED_L)
    Implicit None
    CHARACTER(LEN=*), INTENT(IN) :: MFILE           !mesh file name
    INTEGER, INTENT(INOUT)       :: DIM, NV, NE, NT          !NV nbr vertex, NT nbr triangles
